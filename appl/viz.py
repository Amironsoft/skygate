...
if __name__ == '__main__':
    ...
    for file_name in fnmatch.filter(os.listdir(idir), 'G*13*.json'):
        left_title = ""  # left_title
        title = file_name.replace(".json", "")

        ifile = idir + file_name
        print(ifile)
        ofile = odir + file_name.replace('.json', '_group.html')
        ofile_degree = odir + file_name.replace('.json', '_degree.tsv')
